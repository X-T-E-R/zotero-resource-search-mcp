...
export function createProvider(api: ProviderAPI) {
  return {
    async search(query: string, options?: SearchOptions): Promise<SearchResult> {
      ...
      try {
        const response = await api.http.get<any>(`${BASE}/${startDate}/${endDate}/${cursor}`);
        ...
        if (query && query !== "*" && query.trim()) {
          ...
          items = items.filter(
            (it) =>
              ...
              it.creators?.some((c) =>
                `${c.firstName ?? ""} ${c.lastName}`.toLowerCase().includes(qLower),
              ),
          );
        }
        ...
        api.log.error("medRxiv search failed", error?.message);
        throw error;
      }
    },
  };
...
function parseItem(raw: any): ResourceItem | null {
  ...
  try {
    ...
    const url = `https://www.medrxiv.org/content/${doi}v${version}`;
